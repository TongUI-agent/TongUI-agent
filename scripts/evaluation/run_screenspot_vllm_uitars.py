...
{instruction}"""
    GROUNDING_V2 = "Output only the coordinate of one point in your response. What element matches the following task: "
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "user", "content": 
                [
                    # {"type": "text", "text": GROUNDING.format(instruction=task)},
                    {
                        "type": "image_url",
                        "image_url": {
                    ...
                    {"type": "text", "text": GROUNDING_V2 + task},
                ]
            },
        ],
    ...
    if not prediction_text:
        print("Empty prediction received")
        return None
        
    # Extract coordinates from the action string format
    # Example: "Action: click(start_box='(832,148)')"

    # Extract the coordinates part
    try:
        coords_str = prediction_text
        # Remove parentheses and split into x,y
        x, y = map(int, coords_str.strip('()').split(','))
        return [x / 1000 * img_width, y / 1000 * img_height]
...
def main():
    ...
    LIMIT = -1
    MODEL = "ui-tars"
    ENDPOINT = "http://0.0.0.0:50006/v1"
